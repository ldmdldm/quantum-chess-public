...
use ethers::{
    contract::Contract,
    core::types::{Bytes, TransactionReceipt, U256, transaction::eip2718::TypedTransaction},
...
async fn main() -> Result<()> {
    ...
    println!("Using wallet address: {:#x}", wallet_address);
    ...
    // Parse bytecode into bytes
    let bytecode_bytes = Bytes::from(hex::decode(bytecode.trim_start_matches("0x"))?);
    
    // Create a deployment transaction
    let mut deploy_tx = TypedTransaction::default();
    deploy_tx
        .set_from(wallet_address)
        .set_data(bytecode_bytes);
    
    // Estimate gas with 20% buffer
    let gas_estimate = client.estimate_gas(&deploy_tx, None).await?;
    let gas_buffer = U256::from(12).saturating_mul(gas_estimate);
    let safe_gas_limit = gas_buffer.checked_div(U256::from(10)).unwrap_or(gas_estimate);
    
    println!("Estimated gas for deployment: {}", gas_estimate);
    println!("Using gas limit with safety buffer: {}", safe_gas_limit);
    
    // Send deployment transaction with the gas limit
    deploy_tx.set_gas(safe_gas_limit);
    let pending_tx = client.send_transaction(deploy_tx, None).await?;
